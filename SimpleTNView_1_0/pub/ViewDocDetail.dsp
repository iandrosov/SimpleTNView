...
<body>
	<table width="100%" >
		<tr width="100%">
			<td class="menusection-Logs" colspan=2>Analysis &gt; Transactions &gt; Document Details&nbsp;</td>
		</tr>
...
-->
		...
		%invoke wm.tnweb.queryFlow:viewDocPageFlow%
					...
					%else%
						<li><a href="ViewDocTasks.dsp?docId=%value /docId%">View Document Tasks</a></li>
					%endif resultCount%

					%ifvar relatedDocs%
						%loop relatedDocs%
							%ifvar from vequals(/docId)%
								<li><a href="JavaScript:popUpDoc('ViewDocDetail.dsp?docId=%value to%','relatedFrom');">View Related Document (%value relationship%)</a></li>
							%endif from%
							%ifvar to vequals(/docId)%
								<li><a href="JavaScript:popUpDoc('ViewDocDetail.dsp?docId=%value from%','relatedTo');">View Related Document (%value relationship%)</a></li>
							%endif to%
						%endloop relatedDocs%
					%endif relatedDocs%
				</ul>
