...
<body>
	<table width="100%" >
		<tr width="100%">
			<td class="menusection-Logs2" colspan=2>TNアーカイブ検索結果 &gt; アーカイブドキュメント詳細&nbsp;</td>
		</tr>
...
-->
					<li><a href="ViewArchiveDocActivities.dsp?internalId=%value /docId%">アーカイブ ドキュメント状態遷移ログへ</a></li>

		%invoke SimpleTNView.queryFlow:viewArchiveDocPageFlow%

					%ifvar resultCount equals('0')%
					%else%
						<li><a href="ViewArchiveDocTasks.dsp?docId=%value /docId%">TNアーカイブドキュメントタスクへ</a></li>
					%endif resultCount%

					%ifvar relatedDocs%
						%loop relatedDocs%
							%ifvar from vequals(/docId)%
								<li><a href="JavaScript:popUpDoc('ViewArchiveDocDetail.dsp?docId=%value to%','relatedFrom');">関連ドキュメント (%value relationship%)</a></li>
							%endif from%
							%ifvar to vequals(/docId)%
								<li><a href="JavaScript:popUpDoc('ViewArchiveDocDetail.dsp?docId=%value from%','relatedTo');">関連ドキュメント (%value relationship%)</a></li>
							%endif to%
						%endloop relatedDocs%
					%endif relatedDocs%
				</ul>
